...
export const ListaSocio = ({
  ...
  const [selectedStatus, setSelectedStatus] = useState<{ label: string; color: string }[]>(
    data.map((item) => {
      const matchingOption = STATUS_OPTIONS.find(option => option.label === item.estatus) || STATUS_OPTIONS[3];
